...
import type { LoginRequest, UnlockResultResponse, VerifyTotpRequest } from '../types/auth';
...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    showLoading();
    try {
      const body: LoginRequest = { identifier, masterPassword: password };
      const result = await apiClient.post<UnlockResultResponse>('/auth/login', body);
      ...
      <div style={{ ...cardStyle, width: '100%', maxWidth: 420 }}>
        ...
        {!mfaRequired ? (
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button type="submit" style={{ ...primaryBtnStyle, padding: '12px 0', fontSize: 15, width: '100%' }}>登录</button>

            <p style={{ fontSize: 13, color: colors.textSecondary, textAlign: 'center', margin: 0 }}>
              还没有账户？{' '}
              <a href="#/register" style={{ color: colors.textLink, textDecoration: 'none' }}>去注册</a>
            </p>
          </form>
        ) : (
